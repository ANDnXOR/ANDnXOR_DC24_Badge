...
bool RFM69::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  const uint8_t CONFIG[][2] =
  {
   /* 0x01 */ { REG_OPMODE, RF_OPMODE_SEQUENCER_ON | RF_OPMODE_LISTEN_OFF | RF_OPMODE_STANDBY },
    ...
    /* 0x29 */ { REG_RSSITHRESH, 160 }, // must be set to dBm = (-Sensitivity / 2), default is 0xE4 = 228 so -114dBm
...
//	noInterrupts();
  ...
  uint32_t now = millis();
//Serial.print("RFM69::send() canSend = "); Serial.println(canSend());
//Serial.print("RFM69::send() millis()="); Serial.println(millis());
  while (!canSend() && millis() - now < RF69_CSMA_LIMIT_MS) {
//Serial.print("RFM69::send() millis()="); Serial.println(millis());
//Serial.print("RFM69::send() systick_uptime()="); Serial.println(systick_uptime());
    receiveDone();
  }
...
//Serial.print("Register "); Serial.print(addr,HEX); Serial.print(": "); Serial.println(regval, BIN);
  return regval;
}
